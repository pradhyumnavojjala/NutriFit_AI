...
const ContactsPage = () => {
  ...
  const contactItems = [
    {
      ...
      action: "mailto:pradhyumnavojjala@gmail.com",
      buttonText: "Send Email",
    },
    {
      icon: Phone,
      title: "Hotline (Mon-Fri, 9am-5pm)",
      detail: "+91 (NUTRI) Helpline-No",
      action: "tel:+91 9347017561",
      buttonText: "Call Now",
    },
  ];
  ...
  return (
    ...
    >
      ...
      <div className="relative backdrop-blur-sm border border-border rounded-lg p-6 space-y-10 shadow-lg">
        <CornerElements />
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {contactItems.map((item, index) => (
              <div
                key={index}
                className="border border-border p-5 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-200 space-y-3"
              >
                ...
                >
                  <a href={item.action} aria-label={`Contact via ${item.title}`}>
                    {item.buttonText}
                  </a>
                </Button>
              </div>
        ...
        <div className="space-y-4">
          ...
          <Accordion
            type="single"
            collapsible
            className="w-full border border-border/50 rounded-lg p-4 bg-background/50"
          >
            {faqData.map((item, index) => (
              <AccordionItem key={index} value={`item-${index}`}>
                ...
                <AccordionContent className="text-muted-foreground pt-2 pl-4 border-l-2 border-primary/50 leading-relaxed">
                  {item.answer}
                </AccordionContent>
              </AccordionItem>
            ))}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {resourceItems.map((item, index) => (
              <div
                key={index}
                className="flex justify-between items-center p-4 border border-dashed border-border rounded-lg hover:border-primary/50 transition-colors duration-200"
              >
